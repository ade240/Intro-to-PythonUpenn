...
def import_and_create_bank(filename):
    ...
    for line in lines:
        ...
        #split line into list based on 
        lst = line.strip().split(':')
        
        
        #skip line if does not have a name and deposit amount
        if len(lst) <= 1:
            continue 
            
        #get key (name) and value (deposit amount) from line
        #strip whitespace from beggining of ley (name) and value (deposit amount)
        key = lst[0].strip()
        value = lst[1].strip()
...
def import_and_create_accounts(filename):
    ...
    #iterate over the lines and strip the lines and add to lst serarated by ':'
    for line in lines:
        lst = line.strip().split('-')
...
def change_password(user_accounts, log_in, username, old_password, new_password):
    ...
    - The user is logged in (the username is associated with the value True in the log_in dictionary)
    - The old_password is the user's current password.
    - The new_password should be differnt from the old one.
    - The new_password fulfills the requirement in signup"""
...
def delete_account(user_accounts, log_in, bank, username, password):
    ...
    if(log_in.get(username) and user_accounts.get(username) == password):
        ...
        #another way to delete elemements from dictionary:
        #user__accounts.pop(username)
        #bank.pop(username)
        #log_in.pop(username)
